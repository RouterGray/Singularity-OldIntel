...
BOOL LLPanelTranslationSettings::postBuild()
{
	...
	mBingAPIKeyEditor->setKeystrokeCallback(onBingKeyEdited);
...
// static
void LLPanelTranslationSettings::onBingKeyEdited(LLLineEditor* caller, void*)
{
	if (caller->isDirty())
	{
		LLPanelTranslationSettings::getInstance()->setBingVerified(false, false);
	}
}

// static
void LLPanelTranslationSettings::onGoogleKeyEdited(LLLineEditor* caller, void*)
{
	if (caller->isDirty())
	{
		LLPanelTranslationSettings::getInstance()->setGoogleVerified(false, false);
	}
}
