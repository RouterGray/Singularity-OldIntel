...
BOOL LLFastTimerView::handleMouseDown(S32 x, S32 y, MASK mask)
{
	...
	/* Singu TODO: Add modes like these back?
...
BOOL LLFastTimerView::handleToolTip(S32 x, S32 y, std::string& msg, LLRect* sticky_rect_screen)
{
	...
	if(mPauseHistory && mBarRect.pointInRect(x, y))
	{
		...
		if (mHoverTimer)
		{
			...
			tooltip = get_tooltip(*mHoverTimer, mHoverBarIndex > 0 ? mScrollIndex + mHoverBarIndex : 0, mRecording);
	...
	else
	{
		...
		if (x < mBarRect.mLeft) 
		{
			...
			if (idp)
			{
				tooltip = get_tooltip(*idp, 0, mRecording);
			}
		}
	}
